...
void setup() {
  ...
  PROFILE_COLOUR[0] = #B70000; // red
  PROFILE_COLOUR[3] = #1700B7; // blue
  PROFILE_COLOUR[2] = #B40297; // purple
  PROFILE_COLOUR[1] = #2DB405; // green
  PROFILE_COLOUR[4] = #E38800; // green
...
String[] getTrusteeProfileDetails(int agentId)
{
...
String[] getTrustorProfileDetails(int agentId)
{
...
void draw() 
{
  ...
  text(modeDescription+(currentAgent+1),XSIZE/2,B_LOC_Y+(0.5*PAD));
  textFont(medium);
  ...
  for(int i = 0; i < CURRENT_OPINIONS.length-1; i++)  // length-1??!!
  {
    if(validAgent(currentAgent, MODE)){
      ...
      if(!reset) {
        ...
        {
          strokeWeight(1.5);
          line((int)adjustedX - HOTSPOT_RAD/1.5, (int)adjustedY + HOTSPOT_RAD/1.5, (int)adjustedX + HOTSPOT_RAD/1.5, (int)adjustedY - HOTSPOT_RAD/1.5);
          line((int)adjustedX - HOTSPOT_RAD/1.5, (int)adjustedY - HOTSPOT_RAD/1.5, (int)adjustedX + HOTSPOT_RAD/1.5, (int)adjustedY + HOTSPOT_RAD/1.5);
        }
      }
...
boolean validAgent(int i, int mode)
{
