...
import weka.clusterers.EM; //import weka.clusterers.SimpleKMeans;
 ...
 * 
 */
public class SLClusterer implements Clusterer
{
	 ...
	 * @param opinions
	 */
	public SLClusterer(int noOfClusters)
	{
		isReady = false;
	...
	public void createClusters()
	{
		...
		try {
			...
			// algorithm
			// when labelling the data, since we are using EM, we apply the most
			// likely cluster value
			int[] assignments = new int[data.numInstances()];
			for (int k = 0; k < instancesToAgents.length; k++) {
				// for each agent
				// get the most probable label
				double probs[] = clusterer.distributionForInstance(data.instance(k));
				int bestCluster = 0;
				double bestProb = 0.0;
