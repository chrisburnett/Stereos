 ...
 * This should work for recommenders with perceptual biases, and recommenders
 * who are subject to biased trustor behaviours.
 * 
 * @author cburnett
 * 
 */
public class AntiBiasFilter implements ReputationFilter
{
	...
	/**
	 * Reference to the trustor using this filter, so that we can reason about
	 * its features
	 */
	...
	/**
	 * Interval between recomputing detected biases. We don't want to be doing
	 * this computation every time for every single candidate, but to avoid a
	 * cold start problem this also means we need to be careful of starting too
	 * early and shutting off reputation providers whose only crime is being
	 * uncertain :) Default: 10.
	 */
	private int learningInterval;

	/**
	 * Cached provider lists this model will use to save on recomputing all the
	 * time.
	 */
	...
	/**
	 * Flag to indicate whether the filter is ready yet. If it's not, it just
	 * returns the agents its been given (bypass)
	 */
	private boolean isReady;

	/**
	 * Create a new antibias filter with a threshold value of 0.05 standard
	 * deviation from the mean, and 3 clusters. Learning interval default is 10.
	 * Time to keep agents in memory when they are not seen: 30 invocations.
	 * 
	 * @param trustor
	 ...
	 * 
	 * 1. get opinions 2. if variance is high (defined) 3. then begin clustering
	 * 4. if clusters are found, build a tree to clusters to identify salient
	 * features 5. use tree to classify owner trustor 6. return only agents from
	 * the resulting class.
	 */
	public Map<Agent, List<Agent>> filterRecommenders(Agent self, List<Agent> candidates, List<Agent> recs)
	{
		// only recompute if we are in the learning interval - otherwise return
		// what we have already
		// WARNING - this could result in querying agents that are no longer in
		// the system if we allow
		// for the possibility that trustors could leave, which at the moment we
		// don't.

		// in any event, add the examples to our training data/memory
		// worst this can get is candidates x recommenders (everyone has an
		// opinion)
		...
		if (learningInterval-- > 0) {
			// if the learner hasn't been built yet, bypass, but if it has, use
			// the cache
			if (!isReady)
				return new DefaultFilter().filterRecommenders(self, candidates, recs);

			// if the cache has the sets for the candidates we want, just add
			// them from the cache
		...
		// for each candidate not in cache we're going to come up with a subset
		// of recommenders
		...
		for (Agent a : checkList) {
			...
			// 1. do the opinion query - this is now handled by the memory
			// update
			// function 'addToMemory(), and is already updated by this point
			// now we simply retrieve what we have from memory, opinion query
			// already done
			...
			if (var > threshold) {
				...
				// we are using it in a 'single pass' kind of way so we will be
				// resetting
				// it for each agent, while maintaining a growing set of
				// training data
				// just hope this is better than maintaining a huge heap of
				// models -
				...
				if (learner.isReady()) {
					...
					// int incSize = 0;
					// exclude recommenders who are not in the same cluster as
					// us
					for (Entry<Agent, Integer> labelledAgent : labelledAgents.entrySet()) {
						// System.out.println(labelledAgent.getKey().getId() +
						// ":" + labelledAgent.getValue());
						// all agents in the same cluster as us should be added
						// also, only return agents that were in the original
						// list, otherwise we
						...
						if (labelledAgent.getValue().equals(myCluster) && recs.contains(labelledAgent.getKey())
							...
							// labelledAgent.getKey() + " is in agent " +
							// trustor.getId() + "'s group ("+
							// labelledAgent.getValue());
					...
					// System.out.println("No. excluded agents:" +
					// excludedAgents + " of " + recs.size());
				}
			} else {
				// if the variance is not sufficient to indicate clusters, just
				// return
				// all the recommenders (bypass)
				recList = recs;
			}
			// System.out.print(self.getProfile().getId() + " vs " +
			// a.getProfile().getId() + ":");
			// for(Agent r : recList)
		...
		isReady = true;
		// By -adding- all the new results to the cache, instead of setting the
		// cache
		// to -be- the new results, we're using the cache as a memory to get
		// around the
	...
	/**
	 * Add a new set of opinions about agents to filter's 'memory' -at the
	 * moment, the filter never forgets.
	 * 
	 * @param candidates
	 * @param recs
	 */
	...
	{
		for (Agent a : candidates) {
			// if we've never seen this candidate before, initialise memory for
			// it
			...
			// for each recommender, put its opinion in the map for the current
			// candidate
			for (Agent r : recs)
		...
		for (Agent a : knownAgents) {
			if (!candidates.contains(a)) {
				...
				// if the candidate set does contain this agent, it's still
				// 'live'
